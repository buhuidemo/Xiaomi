var xiaomi = {
	...
	loginJudge() {
		...
		let shoppingCar1 = document.getElementById('shoppingCar1');
		let shoppingCar2 = document.getElementById('shoppingCar2');
		let carNum = document.getElementsByClassName('carNum');
		let html='';
		// 判断是否登录
		if (loginState == 'true') {
			...
			shoppingCar1.style.display = 'none';
			shoppingCar2.style.display = 'block';
			// html += `<a href="shopping.html">
			// 		<em></em>
			// 		购物车
			// 		<span>（0）</span>
			// </a>
			...
			// 	购物车中还没有商品，赶紧选购吧！
			// </div>`
			var nums =localStorage.getItem('key');
			carNum = nums;
			html += `<span class="carNum">${carNum}</span>`;
		} else {
			J_siteInfo.style.display = 'block';
			J_siteUserInfo.style.display = 'none';
			shoppingCar1.style.display = 'block';
			shoppingCar2.style.display = 'none';
	...
	// 封装一个 家电 的方法
	// 实现鼠标移入相应区域内都触发显示出对应页面的事件
	// 即：应该用onmousemove比onmouseover更合理！！！
